...
const filePath = path.join('c:', 'Users', 'ASD', 'Downloads', 'ProjectMERN', 'ProjectMERN', 'firstapp-newone', 'src', 'component', 'Package', 'Packagedetails.tsx');

// Read existing content up to the truncation point
let existing = fs.readFileSync(filePath, 'utf8');
let truncPoint = existing.indexOf('  // Manipur');
if (truncPoint >= 0) {
    existing = existing.substring(0, truncPoint);
}
...
const fullContent = existing + rest;
fs.writeFileSync(filePath, fullContent, 'utf8');
console.log('File restored successfully. Size:', fullContent.length, 'bytes');
